...
###############################################################################################################
$desktop = '~/Desktop/'
$packages = '~/Programs Files/Python/Python311/Lib/site-packages/'
...
# 												 Python Functions  											  #
###############################################################################################################
function pipshow ($package='', $keyword='require') {python -m pip show $package | findstr -i "^$keyword" | findstr -i ":"}
function rewards ([int]$user_agent=0, [int]$word_count=30, [int]$wait=30, [int]$exitbool=0, [int]$test=0, [string]$browser_agent='edge', [string]$loglevel='info') {
	python -c "from rewards import searchbot; searchbot(wait=$wait, words=$word_count, browser_agent=`'$browser_agent`', user_agent=$user_agent, test=$test, loglevel=`'$loglevel`')" `
	-word_count=$word_count -browser_agent=browser_agent -user_agent=$user_agent -wait=$wait -test $test -loglevel $loglevel
	if ($exitbool -eq 1) {exit} }
function rewardsall ([int]$delay=0, [int]$futurebool=0, [int]$num1=30, [int]$num2=20, [int]$bonus=0, [int]$wait2exit=15, [int]$exitbool=1, [int]$test=0, [int]$max_random_time=10, [string]$browser_agent='edge', [string]$loglevel='info', [int]$shuffle=1) {
	python -c "from rewards import searchbots; searchbots(delay=$delay, futurebool=$futurebool, num1=$num1, num2=$num2, bonus=$bonus, wait2exit=$wait2exit, test=$test, max_random_time=$max_random_time, browser_agent=`'$browser_agent`', loglevel=`'$loglevel`', shuffle=$shuffle)" `
	-delay=$delay -futurebool=$futurebool -num1=$num1 -num2=$num2 -bonus=$bonus -wait2exit=$wait2exit -test=$test -max_random_time=$max_random_time -browser_agent=$browser_agent -loglevel=$loglevel -shuffle=$shuffle
	if ($exitbool -eq 1) {exit} }
function rewardsauto ([int]$delay=0, [int]$futurebool=0, [int]$bonus=0, [int]$wait2exit=15, [int]$exitbool=1, [int]$test=0, [int]$max_random_time=10, [string]$browser_agent='edge', [string]$loglevel='info', [int]$shuffle=1) {
	python -c "from rewards import main; main(delay=$delay, futurebool=$futurebool, bonus=$bonus, wait2exit=$wait2exit, test=$test, max_random_time=$max_random_time, browser_agent=`'$browser_agent`', loglevel=`'$loglevel`', shuffle=$shuffle)" `
	-delay=$delay -futurebool=$futurebool -bonus=$bonus -wait2exit=$wait2exit -test=$test -max_random_time=$max_random_time -browser_agent=$browser_agent -loglevel=$loglevel -shuffle=$shuffle
	if ($exitbool -eq 1) {exit} }
